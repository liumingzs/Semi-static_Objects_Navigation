...
class TopologicalMapNode(Node):
    def __init__(self):
        ...
        self.distance_threshold = 0.2  # 节点之间的最小距离
    ...
    def determine_area(self, x, y):
        ...
        if -2 <= x <= 5 and -4 <= y <= 1:
            return "area1"
        elif 5 < x <= 8 and -4 <= y <= 1:
            return "area2"
        elif -2 <= x <= 5 and -10 <= y < -4:
            return "area3"
        elif 5 < x <= 8 and -10 <= y < -4:
            return "area4"
        ...
        if len(data) == 4:
            ...
            if (self.last_object_position is None or object_position != self.last_object_position) :
                ...
                if bool(file_content):
                    ...
                    # 输出节点信息
                    self.get_logger().info(f"Added node with key=\"d{new_node_index}\": {object_position}")
                    self.get_logger().info(f"Color image path with key=\"d{new_node_index}\": {color_image_path}")
        ...
        if not self.nodes:
            ...
            if dist >= self.distance_threshold and dist <= 0.5:
                ...
                for node_id in self.graph.nodes():
                    ...
                    if (node_id < 0 and dist_to_existing <= 0.7):
    ...
    def publish_image_paths(self, node_index):
        ...
        depth_image_path = os.path.join(self.depth_image_dir, f"frame_{node_index:06d}.png")
    ...
    def add_node(self, pose):
        ...
        self.graph.add_node(node_index, pos=pose, image=image_path, area=area)
    ...
    def add_node_to_front(self, pose):      # 加上名称和图片  图片已经加载完成，就差添加名称了
        ...
        # 根据文件内容决定是否添加名称属性
        if file_content:
            self.graph.add_node(node_index, pos=pose, image=image_path, area=area, object_name=file_content)  # 如果文件不为空，添加节点并附带名称
        else:
            self.graph.add_node(node_index, pos=pose, image=image_path, area=area)  # 如果文件为空，仅添加节点
    ...
    def save_images(self, node_index):
        ...
        if self.current_depth_image is not None:
            depth_image_path = os.path.join(self.depth_image_dir, f"frame_{node_index:06d}.png")
            cv2.imwrite(depth_image_path, self.current_depth_image)
