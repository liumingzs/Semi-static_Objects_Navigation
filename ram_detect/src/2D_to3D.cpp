...
private:
    ...
    void checkFlagFile()
    {
        ...
        if (flag_file.is_open())
        {
            ...
            if (flag == "1")
            {
                ...
                std::cout<<color_image_path_<<std::endl;
                ...
                npyReadScript(depth_image_path_);
                // 计算掩码区域的三维坐标点
                ...
                std::array<double, 3> object_position_in_camera = {center.x, -center.y, center.z};  // 可能有问题
                
                // RCLCPP_INFO(this->get_logger(), "current pose: [%f, %f, %f]",
                //             current_pose_.position.x, current_pose_.position.y, current_pose_.position.z);

                // 转换
                auto object_position_in_world = transform_to_world(camera_position, camera_orientation, object_position_in_camera);
                ...
                // 打印消息内容
                std::cout << "Publishing message: " << message.data << std::endl;
    ...
    std::array<double, 3> transform_to_world(const std::array<double, 3>& camera_position,
                                                ...
                                                const std::array<double, 3>& object_position)
        {
            ...
            // 转换到世界坐标系
            Eigen::Vector3d obj_pos_world = cam_ori * obj_pos + cam_pos;
    ...
    cv::Point3f calculate3DPoint(const cv::Point& pt)
        {
            if (pt.y >= 0 && pt.y < color_image_.rows && pt.x >= 0 && pt.x < color_image_.cols)
            {
